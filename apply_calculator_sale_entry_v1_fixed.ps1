...
if ($text -notmatch '\|\s*"manual"\s*;') {
    $pattern = '(export type ProductCategory\s*=\s*(?:\|\s*"[^"]+"\s*)+)(;)'

    if (-not [regex]::IsMatch($text, $pattern)) {
        throw "ProductCategory block not found"
    }
...
if ($text -notmatch 'CalculatorSaleEntry') {
    ...
    $text = $text.Replace(
        $importAnchor,
        'import CalculatorSaleEntry from "../../components/pos/CalculatorSaleEntry";' + "`r`n" + $importAnchor
    )
}

if ($text -notmatch 'const addCalculatorAmount') {
    $pattern = '(\s+const addReturnLines\s*=\s*\(\s*lines:\s*CartLine\[\],\s*\)\s*=>\s*\{)'
...
if ($text -notmatch '<CalculatorSaleEntry') {
    $pattern = '(<div className="sale-page__content">\s*)(<section className="sale-page__catalog">)'
    ...
    $replacement = @'
$1{activeProfile.operatingModel ===
                        "calculator" && (
                        <CalculatorSaleEntry
                            onAddAmount={
                                addCalculatorAmount
