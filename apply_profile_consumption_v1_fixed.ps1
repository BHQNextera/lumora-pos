...
$componentPattern =
    'function SalePage\(\{\s*incomingReturnLines = \[\],\s*onReturnLinesConsumed,\s*\}: SalePageProps\) \{\s*const \{ products \} = useCatalog\(\);'
...
if (-not [regex]::IsMatch($text, $componentPattern)) {
...
# ============================================================
# 3. VERIFY WITHOUT NON-ASCII POWERSHELL PATTERNS
# ============================================================
...
if ($sidebarCheck -notmatch 'getActiveBusinessOperatingProfile') {
    throw "Sidebar active profile connection missing"
}

if ($sidebarCheck -match 'const navigationItems: SidebarItem\[\] = \[' -and
    $sidebarCheck -notmatch 'profile\.features\.catalog') {
    throw "Sidebar capability filtering missing"
}
...
if ($saleCheck -notmatch 'const posCapabilities =\s*activeProfile\.pos;') {
    throw "SalePage active profile capabilities missing"
}
